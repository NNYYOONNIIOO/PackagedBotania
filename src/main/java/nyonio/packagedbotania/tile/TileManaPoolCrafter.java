...
import nyonio.packagedbotania.recipe.IRecipeInfoManaPool;
...
public class TileManaPoolCrafter extends TileAE2Base implements ITickable, IPackageCraftingMachine, ISparkAttachable {
    ...
    protected IRecipeInfoManaPool currentRecipe;
    ...
    public TileManaPoolCrafter() {
        setInventory(new InventoryTileBase(this, 2));
        setEnergyStorage(new EnergyStorage(this, energyCapacity));
    }

    @Override
    protected String getLocalizedName() {
        return I18n.translateToLocal("tile.packaged_botania.mana_pool_crafter.name");
    }

    @Override
    protected ItemStack getStackRepresentation() {
        return new ItemStack(BlockManaPoolCrafter.ITEM_INSTANCE);
    }
                ...
                if(energyStorage.extractEnergy(energyUsage, true) >= energyUsage ||
                   extractEnergyFromNetwork(energyUsage, true) >= energyUsage) {
                    ...
                    if(fromStorage < energyUsage) {
                        fromAE2 = extractEnergyFromNetwork(energyUsage - fromStorage, false);
                    }
    ...
    protected void finishProcess() {
        ...
        inventory.stacks.set(0, ItemStack.EMPTY);
        inventory.stacks.set(1, currentRecipe.getOutput().copy());
    ...
    protected void ejectItems() {
        ItemStack stack = inventory.stacks.get(1);
        if(!stack.isEmpty()) {
            ItemStack rem = ejectItemToNetwork(stack);
            inventory.stacks.set(1, rem);
        }
    }
    ...
    protected void chargeEnergy() {
        ...
        if(energyRequest > 0) {
            ItemStack energyStack = inventory.stacks.get(0);
    ...
    public boolean acceptPackage(IRecipeInfo recipeInfo, List<ItemStack> stacks, EnumFacing facing) {
        if(!isBusy() && recipeInfo.isValid() && recipeInfo instanceof IRecipeInfoManaPool) {
            IRecipeInfoManaPool recipe = (IRecipeInfoManaPool)recipeInfo;
            IBlockState stateBelow = world.getBlockState(pos.down());
            IBlockState requiredCatalyst = recipe.getCatalyst();

            boolean catalystValid = false;
            if(requiredCatalyst == null) {
                catalystValid = stateBelow.getBlock() != ModBlocks.alchemyCatalyst &&
                               stateBelow.getBlock() != ModBlocks.conjurationCatalyst;
            } else {
                catalystValid = stateBelow == requiredCatalyst;
            }
            ...
            if(catalystValid) {
                ...
                inventory.stacks.set(0, recipe.getInput().copy());
                markDirty();
                syncTile(false);
                return true;
            }
    ...
    public boolean isBusy() {
        return isWorking || !inventory.stacks.get(1).isEmpty();
    }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        if(nbt.hasKey("Recipe")) {
            ...
            if(recipe instanceof IRecipeInfoManaPool) {
                currentRecipe = (IRecipeInfoManaPool)recipe;
            }
        }
